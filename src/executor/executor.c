...
void	simple_command(t_data *data, t_process *process)
{
...
void	link_command(t_data *data, t_commands *cmds, t_process *process,
...
void	execute_all(t_data *data, t_commands *cmds)
{
	...
	if (!is_builtin(NULL, cmds->args[0]) || num_cmd > 1)
	{
		...
		while (cmds)
		{
			link_command(data, cmds, &process, pipe_read_end_prev);
